...
This script generates the PolicyGenerator, then processes it to create Policy, Placement, and PlacementBinding resources (PlacementBinding is always included).
...
Options:
  ...
  -p, --process              Process PolicyGenerator to create Policy/Placement/PlacementBinding (default: true)
...
Config File Format (YAML):
  ...
  clusterSets:
    - <cluster-set-name1>
    - <cluster-set-name2>
  # Note: PlacementBinding is always included regardless of option chosen
...
# Process PolicyGenerator to create Policy, Placement, and PlacementBinding
...
try:
    ...
    binding_name = policygen.get('placementBindingDefaults', {}).get('name', f"binding-{policy_name}")
    ...
    # Generate PlacementBinding (always included)
    binding = {
        'apiVersion': 'policy.open-cluster-management.io/v1',
    ...
    resources = [policy, placement_resource, binding]
...
EOF
    ...
    if [ "$PROCESS_POLICYGEN" = "true" ]; then
        ...
        # Pass cluster sets info to processing function
        ...
        if [ -f "$processed_file" ]; then
            ...
            if [ -n "$CLUSTER_SETS" ]; then
                echo -e "${BLUE}   Note: Using cluster sets${NC}"
            fi
        fi
    else
